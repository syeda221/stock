...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
    <div>
        ...
        <small class="text-muted">Manage product classification and grouping</small>
    </div>

    <div class="d-flex align-items-center gap-2 mt-2 mt-md-0">
        {{-- Search (UI only, backend optional) --}}
        ...
        <a href="{{ route('product-category.create') }}"
           class="btn btn-sm btn-primary rounded-pill shadow-sm px-3">
            <i class="bi bi-plus-lg me-1"></i> Add Product Category
        </a>
    </div>
</div>
...
<div class="card border-0 shadow-sm rounded-4 mb-3">
    <div class="card-body p-3">
        <form id="categoryFilterForm" class="mb-0">
            <div class="row g-2">
                <div class="col-md-6">
                    <label class="form-label fw-semibold small">Search</label>
                    <input type="text" name="search" id="filter_search" class="form-control form-control-sm filter-field" placeholder="Search category name...">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <span class="ms-2 text-muted small align-self-center">Total: <strong id="totalCount">{{ $categories->count() }}</strong></span>
                    </div>
                </div>
            </div>
        </form>
                            ...
                            <td class="text-end">
                                <div class="btn-group">
                                    ...
                                    <form action="{{ route('product-category.destroy', $category->id) }}"
                                          method="POST"
                                          class="d-inline"
                                          onsubmit="return confirm('Delete this product category?')">
                                        @csrf
                                        @method('DELETE')
    ...
    function applyFilters() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (newTableBody) {
                    $('#categoryTableBody').html(newTableBody.innerHTML);
                }
