...
<div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
    <div>
        ...
        <h5 class="mb-0 fw-bold">Units of Measure</h5>
        <small class="text-muted">Manage your measurement units (kg, pcs, box, etc.)</small>
    </div>

    <div class="d-flex align-items-center gap-2 mt-2 mt-md-0">
        {{-- Optional search UI (frontend only for now) --}}
        ...
        <a href="{{ route('uom.create') }}" class="btn btn-sm btn-primary rounded-pill shadow-sm px-3">
...
<div class="card border-0 shadow-sm rounded-4 mb-3">
    <div class="card-body p-3">
        <form id="uomFilterForm" class="mb-0">
            <div class="row g-2">
                <div class="col-md-6">
                    <label class="form-label fw-semibold small">Search</label>
                    <input type="text" name="search" id="filter_search" class="form-control form-control-sm filter-field" placeholder="Search UOM name...">
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <span class="ms-2 text-muted small align-self-center">Total: <strong id="totalCount">{{ $uoms->count() }}</strong></span>
                    </div>
                </div>
            </div>
        </form>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody id="uomTableBody">
                    @forelse($uoms as $uom)
                        <tr>
                            <td class="text-muted fw-semibold">{{ ($uoms->currentPage() - 1) * $uoms->perPage() + $loop->iteration }}</td>

                            <td>
                                <div class="fw-semibold text-dark">{{ $uom->name }}</div>
                                <small class="text-muted">UOM ID: {{ $uom->id }}</small>
                            </td>
                            ...
                            <td class="text-end">
                                <div class="btn-group">
                                    ...
                                    <form action="{{ route('uom.destroy', $uom->id) }}"
                                          method="POST"
                                          class="d-inline"
        ...
        <div class="card-footer bg-light border-top py-3">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                <div class="text-muted small">
                    ...
                    Showing <strong>{{ $uoms->firstItem() }}</strong> to <strong>{{ $uoms->lastItem() }}</strong> of <strong>{{ $uoms->total() }}</strong> entries
                </div>
                <div>
    ...
    function applyFilters() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const newTableBody = doc.querySelector('#uomTableBody');
                if (newTableBody) {
                    $('#uomTableBody').html(newTableBody.innerHTML);
                }
